...
exports.createCoordinator = async (req, res) => {
  const {
    ...
    phone_number,
    profile_image_url,
    address,   // ⬅️ now accepted
    district,  // ⬅️ now accepted
  } = req.body;

  // include address & district in required fields
  if (!full_name || !email || !password || !phone_number || !address || !district) {
...
exports.updateCoordinator = async (req, res) => {
  ...
  // allow updating address & district too
  const allowed = [
    'full_name',
    'email',
    'phone_number',
