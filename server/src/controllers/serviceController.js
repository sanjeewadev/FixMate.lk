...
exports.updateService = async (req, res) => {
  try {
    ...
    if (updates.slug) {
      ...
      const exists = await Service.exists({ _id: { $ne: id }, slug: updates.slug });
    ...
    if (updates.imageUrls) {
      const urls = Array.isArray(updates.imageUrls) ? updates.imageUrls : [updates.imageUrls];
      newImages.push(...urls.filter(Boolean).map(u => ({ public_id: null, url: u })));
    ...
    if (newImages.length > 0) {
      if (String(updates.replaceImages || '').toLowerCase() === 'true') {
        ...
        updates.serviceImages = [...(current?.serviceImages || []), ...newImages];
      }
