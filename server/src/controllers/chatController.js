  ...
  try {
    ...
    if (bookingId) {
      if (!mongoose.isValidObjectId(bookingId))
        return res.status(400).json({ message: "bookingId invalid" });
    ...
    const existing = await ChatConversation.findOne({
      ...
      $and: [
        {
          participants: {
            $elemMatch: { role: req.user.role, userId: req.user.id },
          },
        },
        {
...
exports.postMessage = async (req, res) => {
  try {
    ...
    let isParticipant = (convo.participants || []).some(
      (p) =>
        String(p.userId) === String(req.user.id) && p.role === req.user.role,
    );
...
exports.listConversations = async (req, res) => {
  try {
    ...
    const raw = await ChatConversation.find(q)
      .sort({ updatedAt: -1 })
