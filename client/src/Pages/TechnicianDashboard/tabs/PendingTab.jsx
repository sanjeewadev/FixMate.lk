import React, { useCallback, useEffect, useState } from "react";
import { format } from "date-fns";
import { Clock3, RefreshCw } from "lucide-react";

import api from "../../../lib/api";
import "./ApprovedTab.css";

const fmt = (value) => {
  if (!value) return "—";

  try {
    return format(new Date(value), "PPpp");
  } catch {
    return "—";
  }
};

export default function PendingTab() {
  const [pending, setPending] = useState([]);
  const [loading, setLoading] = useState(false);
  const [err, setErr] = useState("");

  const loadPending = useCallback(async () => {
    try {
      setLoading(true);
      setErr("");

      const { data } = await api.get(
        "/api/technician/bookings/mine?status=awaiting_coordinator",
      );

      setPending(Array.isArray(data) ? data : []);
    } catch (error) {
      setPending([]);
      setErr(
        error?.response?.data?.message ||
          "Failed to load pending coordinator approval records.",
      );
    } finally {
      setLoading(false);
    }
  }, []);

  useEffect(() => {
    loadPending();
  }, [loadPending]);

  return (
    <section className="fm-tech-tabs">
      <div className="fm-tech-tabs__header">
        <div>
          <span className="fm-tech-tabs__eyebrow">Waiting Queue</span>
          <h1>Pending Coordinator Approval</h1>
          <p>
            These bookings were accepted by you and are now waiting for
            coordinator confirmation.
          </p>
        </div>

        <button
          type="button"
          className="fm-tech-tabs__btn fm-tech-tabs__btn--outline"
          onClick={loadPending}
          disabled={loading}>
          <RefreshCw size={16} />
          {loading ? "Refreshing" : "Refresh"}
        </button>
      </div>

      {err ? (
        <div className="fm-tech-tabs__notice fm-tech-tabs__notice--error">
          {err}
        </div>
      ) : null}

      <section className="fm-tech-tabs__card">
        <div className="fm-tech-tabs__cardHeader">
          <div>
            <span>Pending records</span>
            <h2>Awaiting Coordinator</h2>
          </div>
        </div>

        <div className="fm-tech-tabs__tableWrap">
          <table className="fm-tech-tabs__table">
            <thead>
              <tr>
                <th>Service</th>
                <th>Problem</th>
                <th>Preferred Date</th>
              </tr>
            </thead>

            <tbody>
              {pending.length === 0 ? (
                <tr>
                  <td colSpan={3}>
                    <div className="fm-tech-tabs__empty">
                      <Clock3 size={24} />
                      <strong>No pending approvals</strong>
                      <span>
                        No bookings are waiting for coordinator approval.
                      </span>
                    </div>
                  </td>
                </tr>
              ) : (
                pending.map((booking) => (
                  <tr key={booking._id}>
                    <td>{booking.service?.name || "—"}</td>

                    <td>
                      <div className="fm-tech-tabs__titleCell">
                        <strong>{booking.problemTitle || "—"}</strong>
                        <small>{booking._id}</small>
                      </div>
                    </td>

                    <td>{fmt(booking.preferredAt)}</td>
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </div>
      </section>
    </section>
  );
}
