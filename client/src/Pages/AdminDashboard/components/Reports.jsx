...
/* utils */
function num(n){ if(n==null) return "0"; return n.toLocaleString(undefined,{maximumFractionDigits:2}); }
...
function Sparkline({ points=[], height=48, padding=4, strokeWidth=2 }){
  ...
  const sx = (i)=> padding + (i*(w-padding*2))/Math.max(points.length-1,1);
  const sy = (v)=> padding + (height-padding*2)*(1-(v-min)/range);
...
export default function Reports(){
  ...
  async function loadAll(){
    try{
      ...
      const [sumRes,svcRes,distRes,techRes,highRes,dailyRes,monthlyRes] = await Promise.all([
  ...
  useEffect(()=>{ loadAll(); /* eslint-disable-next-line */ },[qs]);

  if(!isAdmin) return <div className="msg error">You are not authorized to view Reports.</div>;
    ...
    <div className="reports-page">
      ...
      <div className="filters-bar">
        ...
        <div className="filter">
          ...
          <input type="number" step="0.01" min="0" max="1" placeholder="(optional, 0..1)"
      ...
      <div className="kpi-grid">
        <div className="metric-card"><h4>Total Revenue</h4><div className="metric">LKR {num(summary?.totalRevenue)}</div></div>
        <div className="metric-card"><h4>Total Expenses</h4><div className="metric">LKR {num(summary?.totalExpenses)}</div></div>
        <div className="metric-card"><h4>Service Charges</h4><div className="metric">LKR {num(summary?.totalServiceCharge)}</div></div>
        <div className="metric-card"><h4>Projected Profit</h4><div className="metric">LKR {num(summary?.totalProfit)}</div></div>
        <div className="metric-card"><h4>Completed Bookings</h4><div className="metric">{num(summary?.count)}</div></div>
      </div>

      {/* Highest booking (stacked) */}
      <section className="report-section">
        <div className="section-head"><h3>Highest Grossing Booking</h3></div>
      ...
      {/* Monthly sparkline (stacked) */}
      <section className="report-section">
        <div className="section-head"><h3>Monthly Revenue Trend</h3></div>
        <div className="metric-card chart-card">
          <Sparkline points={monthly.map(m=>({x:m.yearMonth, y:m.revenue||0}))}/>
          <div className="muted tiny">{monthly.map(m=>m.yearMonth).join(" · ")}</div>
        </div>
      </section>

      {/* Top Services (stacked) */}
      <section className="report-section">
        <div className="section-head"><h3>Top Services</h3><div className="muted">Sorted by Revenue</div></div>
        <div className="table-wrapper">
          <table className="styled-table">
            <thead><tr>
              <th>#</th><th>Service</th><th>Count</th><th>Revenue (LKR)</th>
              <th>Expenses (LKR)</th><th>Service Charges (LKR)</th><th>Profit (LKR)</th>
            </tr></thead>
            <tbody>
              {topServices.length ? topServices.map((r,i)=>(
                <tr key={r.serviceId||i}>
                  ...
                  <td>{num(r.count)}</td><td>{num(r.revenue)}</td><td>{num(r.expenses)}</td>
                  <td>{num(r.serviceCharge)}</td><td><b>{num(r.profit)}</b></td>
                </tr>
      ...
      <section className="report-section">
        <div className="section-head"><h3>Top Districts</h3><div className="muted">By Revenue</div></div>
        <div className="table-wrapper">
          <table className="styled-table">
            <thead><tr><th>#</th><th>District</th><th>Count</th><th>Revenue</th><th>Profit</th></tr></thead>
            <tbody>
              {topDistricts.length ? topDistricts.map((r,i)=>(
                <tr key={r.district||i}>
                  <td>{i+1}</td><td>{r.district||"-"}</td><td>{num(r.count)}</td>
                  <td>{num(r.revenue)}</td><td><b>{num(r.profit)}</b></td>
                </tr>
      ...
      {/* Top Technicians (stacked) */}
      <section className="report-section">
        <div className="section-head"><h3>Top Technicians</h3><div className="muted">By Revenue</div></div>
        <div className="table-wrapper">
          <table className="styled-table">
            <thead><tr><th>#</th><th>Technician</th><th>Count</th><th>Revenue</th><th>Profit</th></tr></thead>
            <tbody>
              {topTechnicians.length ? topTechnicians.map((r,i)=>(
                <tr key={r.technicianId||i}>
                  ...
                  <td>{num(r.count)}</td><td>{num(r.revenue)}</td><td><b>{num(r.profit)}</b></td>
                </tr>
              )) : <tr><td colSpan="5" className="muted">No data.</td></tr>}
            </tbody>
          </table>
      ...
      {/* Daily (stacked) */}
      <section className="report-section">
        <div className="section-head"><h3>Daily (Revenue)</h3><div className="muted">Sum by day</div></div>
        <div className="table-wrapper">
          <table className="styled-table">
            <thead><tr><th>Date</th><th>Revenue</th><th>Expenses</th><th>Profit</th><th>Count</th></tr></thead>
            <tbody>
              {daily.length ? daily.map(d=>(
                <tr key={d.date}>
                  <td>{d.date}</td><td>{num(d.revenue)}</td><td>{num(d.expenses)}</td>
                  <td><b>{num(d.profit)}</b></td><td>{num(d.count)}</td>
                </tr>
              )) : <tr><td colSpan="5" className="muted">No data.</td></tr>}
            </tbody>
          </table>
      ...
      {/* Monthly (stacked) */}
      <section className="report-section">
        <div className="section-head"><h3>Monthly (Revenue)</h3><div className="muted">Sum by month</div></div>
        <div className="table-wrapper">
          <table className="styled-table">
            <thead><tr><th>Month</th><th>Revenue</th><th>Expenses</th><th>Profit</th><th>Count</th></tr></thead>
            <tbody>
              {monthly.length ? monthly.map(m=>(
                <tr key={m.yearMonth}>
                  <td>{m.yearMonth}</td><td>{num(m.revenue)}</td><td>{num(m.expenses)}</td>
                  <td><b>{num(m.profit)}</b></td><td>{num(m.count)}</td>
                </tr>
              )) : <tr><td colSpan="5" className="muted">No data.</td></tr>}
            </tbody>
          </table>
