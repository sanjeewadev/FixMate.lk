    ...
    <div className="ms">
      ...
      <div className="ms-header">
        <div className="ms-title">
          ...
          <div className="ms-sub">Create, update, activate/deactivate, or delete services.</div>
        </div>
      </div>
      ...
      {/* Form Card */}
      <form className="ms-card ms-form" onSubmit={handleSubmit} autoComplete="off">
        ...
        <div className="ms-form-grid">
          ...
          <div className="ms-fields">
            <div className="ms-cols-2">
              <div className="ms-field">
                ...
                <input name="name" type="text" placeholder="e.g., AC Repair" value={form.name} onChange={handleChange} required />
              </div>
              <div className="ms-field">
                <label>Category</label>
                <input name="category" type="text" placeholder="e.g., Electrical" value={form.category} onChange={handleChange} />
              </div>
            </div>
            ...
            <div className="ms-cols-2">
              <div className="ms-field">
                ...
                <input name="basePrice" type="number" min="0" placeholder="e.g., 2500" value={form.basePrice} onChange={handleChange} />
              </div>
            </div>

            <div className="ms-field">
              <label>Description</label>
              <textarea name="description" rows={4} placeholder="Short description…" value={form.description} onChange={handleChange} />
            </div>
            ...
            <div className="ms-actions">
              ...
              {editingId && (
                <button type="button" className="ms-btn ms-btn--outline" onClick={resetForm}>
                  Cancel
                </button>
              )}
            </div>
          ...
          <div className="ms-avatar-card">
            ...
            <div className="ms-avatar-actions">
              ...
              <div className="tiny muted">Square images recommended · JPG/PNG</div>
            </div>
          </div>
        </div>
      </form>
      ...
      <div className="ms-card">
        <div className="ms-list-head">
          ...
          <div className="ms-list-controls">
            ...
            <button className="ms-btn ms-btn--outline" onClick={fetchServices} disabled={loading}>
        ...
        <div className="ms-cards">
          {list.map((s) => {
            ...
            return (
              <div key={s._id} className={`ms-card-item ${inactive ? "inactive" : ""}`}>
                {img ? <img src={img} className="ms-card-thumb" alt={s.name} /> : <div className="ms-card-thumb placeholder" />}
                <h4>{s.name}</h4>
                <p className="muted">{s.category || "—"}</p>
                <p className="muted">{s.basePrice != null ? `Rs. ${s.basePrice}` : "—"}</p>
                <p>Status: {inactive ? "❌ Inactive" : "✅ Active"}</p>
                <div className="ms-row-actions">
                  <button className="ms-btn ms-btn--small ms-btn--primary" onClick={()=>startEdit(s)}>Edit</button>
                  {inactive ? (
                    <button className="ms-btn ms-btn--small ms-btn--outline" onClick={()=>handleActivate(s._id)}>Activate</button>
                  ) : (
                    <button className="ms-btn ms-btn--small ms-btn--warn" onClick={()=>handleSoftDelete(s._id)}>Deactivate</button>
                  )}
                  <button className="ms-btn ms-btn--small ms-btn--danger" onClick={()=>handleHardDelete(s._id)}>Delete</button>
                </div>
              </div>
            );
          })}
          {!list.length && !loading && <div className="muted tiny" style={{padding:"6px 2px"}}>No services</div>}
        </div>
      </div>
          ...
          <div className="ms-crop-modal" onClick={(e)=>e.stopPropagation()}>
            ...
            <div className="ms-crop-controls">
              ...
              <div className="ms-crop-actions">
                <button type="button" className="ms-btn ms-btn--outline" onClick={cleanupCrop}>Cancel</button>
                <button type="button" className="ms-btn ms-btn--primary" onClick={confirmCrop}>Use Image</button>
              </div>
            </div>
          </div>
        </div>
