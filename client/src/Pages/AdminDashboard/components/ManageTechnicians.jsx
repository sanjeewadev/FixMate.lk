...
export default function ManageTechnicians() {
  ...
  async function loadApplications(includeConverted = false) {
    try {
      ...
      const { data } = await api.get(
        `/api/admin/technicians/applications${includeConverted ? "?includeConverted=1" : ""}`,
      );
  ...
  async function toggleSuspend(t) {
    try {
      if (t.is_suspended)
        await api.post(`/api/admin/technicians/${t._id}/unsuspend`);
      else await api.post(`/api/admin/technicians/${t._id}/suspend`);
      loadTechnicians();
    } catch (e) {
      setMsg(e?.response?.data?.message || "Failed to update suspension");
    }
  }
  ...
  async function convertApplication(id) {
    ...
    try {
      ...
      setMsg(e?.response?.data?.message || "Error converting application");
    }
  }
  ...
  async function deleteApplication(id) {
    ...
    try {
      ...
      setMsg(e?.response?.data?.message || "Failed to delete application");
    }
  }
      ...
      <div className="mt-header">
        <div className="mt-title">
          ...
          <div className="mt-sub">
            Create, update, suspend, and convert applications. Avatars are
            optional.
          </div>
        </div>
      </div>
            ...
            <div className="mt-field">
              ...
              {editId && (
                <div className="tiny muted">
                  Changing email updates the technician’s login.
                </div>
              )}
            </div>
              ...
              {technicians.map((t) => (
                <tr key={t._id}>
                  <td>
                    {t.full_name}
                    {t.is_suspended && (
                      <span className="mt-badge mt-badge--red">Suspended</span>
                    )}
                  </td>
                  ...
                  <td className="mt-row-actions">
                    <button
                      className="mt-btn mt-btn--small mt-btn--primary"
                      onClick={() => handleEdit(t)}>
                      Edit
                    </button>
                    <button
                      className="mt-btn mt-btn--small mt-btn--secondary"
                      onClick={() => toggleSuspend(t)}>
                      {t.is_suspended ? "Unsuspend" : "Suspend"}
                    </button>
                    <button
                      className="mt-btn mt-btn--small mt-btn--danger"
              ...
              {applications.map((a) => (
                <tr key={a._id}>
                  <td>
                    {a.full_name}
                    {a.status === "converted" && (
                      <span className="mt-badge mt-badge--green">
                        Converted
                      </span>
                    )}
                  </td>
                  ...
                  <td className="mt-row-actions">
                    {a.status !== "converted" && (
                      <button
                        className="mt-btn mt-btn--small mt-btn--secondary"
                        onClick={() => convertApplication(a._id)}>
                        Convert
                      </button>
                    )}
                    <button
        ...
        <div
          ...
          <div className="mt-crop-modal" onClick={(e) => e.stopPropagation()}>
            <div className="mt-crop-head">
              ...
              <button
                type="button"
                className="mt-btn mt-btn--danger mt-btn--small"
                onClick={cleanupCrop}>
                Close
              </button>
            </div>
            ...
            <div className="mt-crop-controls">
              ...
              <div className="mt-crop-actions">
                <button
                  type="button"
                  className="mt-btn mt-btn--danger mt-btn--outline"
                  onClick={cleanupCrop}>
                  Cancel
                </button>
                <button
